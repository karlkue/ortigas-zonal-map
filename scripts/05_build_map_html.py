...
cbd_ring_json = json.dumps(boundaries["ortigasCbdPerimeter"])
sa_ring_json = json.dumps(boundaries["barangaySanAntonioPerimeter"])
hh_ring_json = json.dumps(boundaries["barangayHighwayHillsPerimeter"])
...
<body>
  ...
  <div id="rate-hud" class="floating-card rate-hud">
    <div class="rate-hud-header" onclick="toggleRateHud()">
      ...
      <button class="rate-hud-toggle" id="rate-hud-btn">Collapse</button>
    </div>
    <div class="rate-hud-body" id="rate-hud-body">
      <div class="rate-gradient-bar" id="rate-hud-gradient"></div>
  ...
  <script>
    const cbdPolygonCoords = {cbd_ring_json};
    const sanAntonioCoords = {sa_ring_json};
    const highwayHillsCoords = {hh_ring_json};
    const properties = {properties_json};
    const bboxBounds = {bbox_bounds_json};
    ...
    const map = L.map('map', {{ layers: [osm], zoomControl: false }}).fitBounds(bboxBounds, {{ padding: [30, 30] }});
    ...
    function getPropertyStyle(p, mode) {{
      ...
      if (mode === 'CR') {{
        ...
        else if (v >= 240000) c = '#2563eb';
        else if (v >= 180000) c = '#0284c7';
        else if (v >= 100000) c = '#0d9488';
        else c = '#10b981';
        ...
        const v = p.rrVal;
        let c = '#65a30d';
        if (v >= 180000) c = '#b91c1c';
        else if (v >= 120000) c = '#ea580c';
        else if (v >= 80000) c = '#d97706';
        else if (v >= 45000) c = '#ca8a04';
        return {{ fillColor: c, fillOpacity: 0.82, color: '#1e293b', weight: 1.1 }};
      }} else if (mode === 'RC') {{
        const v = p.rcVal;
        let c = '#84cc16';
        if (v >= 225000) c = '#991b1b';
        else if (v >= 150000) c = '#ea580c';
        else if (v >= 100000) c = '#eab308';
        ...
        else if (v >= 110000) c = '#059669';
        else if (v >= 80000) c = '#10b981';
        return {{ fillColor: c, fillOpacity: 0.82, color: '#1e293b', weight: 1.1 }};
      }} else {{
    ...
    function getPopupHtml(p, mode) {{
      ...
      if (mode === 'CR') {{
        ...
        if (!p.ccVal) {{
          ...
          activeRateDisplay = `
            <div style="background:#dbeafe; border:1px solid #bfdbfe; padding:8px 10px; border-radius:6px; margin:8px 0;">
              <div style="font-size:11px; font-weight:700; color:#1e40af; text-transform:uppercase;">Active Layer: CC Office Rate</div>
              <div style="font-size:17px; font-weight:800; color:#1e3a8a; margin-top:2px;">₱` + p.ccVal.toLocaleString() + ` / sqm</div>
    ...
    properties.forEach(p => {{
      ...
      poly.on('mouseover', function() {{
        if (isPropertyApplicable(this.propertyData, currentLayerMode)) {{
          this.setStyle({{ weight: 2.5, color: '#f59e0b' }});
        }}
      }});

      poly.on('mouseout', function() {{
        this.setStyle(getPropertyStyle(this.propertyData, currentLayerMode));
      }});
    ...
    function updateActiveLayer(mode) {{
      ...
      document.querySelectorAll('.layer-radio-row').forEach(r => r.classList.remove('active'));
      const activeRow = document.getElementById('row-' + mode);
      if (activeRow) activeRow.classList.add('active');

      propertyPolygons.forEach(poly => {{
        poly.setStyle(getPropertyStyle(poly.propertyData, mode));
        applyPolygonInteractivity(poly, mode);
      }});
    ...
    function toggleRateHud() {{
      ...
      const btn = document.getElementById('rate-hud-btn');
    ...
    function renderRateHud(mode) {{
      ...
      if (mode === 'CR') {{
        ...
        grad.style.background = 'linear-gradient(to right, #10b981, #0d9488, #0284c7, #2563eb, #7e22ce)';
        tiers.innerHTML = `
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#7e22ce;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱315,000 – ₱325,000 / sqm</span></div><div class="rate-tier-desc">ADB Ave, San Miguel Ave, Julia Vargas, Meralco Ave</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#2563eb;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱240,000 – ₱290,000 / sqm</span></div><div class="rate-tier-desc">Emerald, Garnet, Sapphire, Ruby, EDSA Manda, Pearl Dr</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#0284c7;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱180,000 – ₱200,000 / sqm</span></div><div class="rate-tier-desc">EDSA Highway Hills (₱200k), Escriva, Amethyst, Shaw Manda</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#0d9488;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱100,000 – ₱150,000 / sqm</span></div><div class="rate-tier-desc">Shaw Blvd (₱150k), Mayflower/Reliance/Sheridan/Pioneer (₱100k)</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#10b981;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱45,000 – ₱90,000 / sqm</span></div><div class="rate-tier-desc">Williams (₱90k), Samat (₱75k), Sierra Madre (₱65k), Baseline (₱45k)</div></div></div>
        `;
      }} else if (mode === 'CC') {{
        ...
        tiers.innerHTML = `
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#581c87;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱220,000 – ₱315,000 / sqm</span></div><div class="rate-tier-desc">Galleon (₱315k), Spectrum (₱294k), One Sierra (₱220k)</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#2563eb;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱150,000 – ₱205,000 / sqm</span></div><div class="rate-tier-desc">St. Francis (₱189k), Zitan (₱170k), Discovery, Lancaster (₱133k)</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#06b6d4;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱120,000 – ₱149,000 / sqm</span></div><div class="rate-tier-desc">Tektite (₱141k), Citynet Central (₱130k), Soho Central (₱130k)</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#10b981;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱70,000 – ₱110,000 / sqm</span></div><div class="rate-tier-desc">California Garden (₱95k), Facilities Center (₱93k), Summit One (₱82k)</div></div></div>
        ...
        tiers.innerHTML = `
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#b91c1c;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱180,000 – ₱200,000 / sqm</span></div><div class="rate-tier-desc">EDSA Mandaluyong Residential Corridor</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#ea580c;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱120,000 – ₱147,000 / sqm</span></div><div class="rate-tier-desc">Ortigas Ave, Julia Vargas, Shaw, Wack-Wack Road</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#d97706;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱100,000 – ₱110,000 / sqm</span></div><div class="rate-tier-desc">Pasig San Antonio Base (₱100k), Corinthian / Ugong Norte (₱110k)</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#ca8a04;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱50,000 – ₱78,000 / sqm</span></div><div class="rate-tier-desc">Palladium Subd (₱78k), Samat (₱60k), Sierra Madre / Old Wack-Wack (₱50k)</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#65a30d;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱34,000 – ₱48,000 / sqm</span></div><div class="rate-tier-desc">DM Guevarra (₱42k), Mariveles (₱41k), Highway Hills Baseline (₱34k)</div></div></div>
        `;
      }} else if (mode === 'RC') {{
        ...
        tiers.innerHTML = `
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#991b1b;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱205,000 – ₱275,000 / sqm</span></div><div class="rate-tier-desc">Galleon (₱275k), Spectrum (₱245k), Sapphire (₱225k), One Sierra (₱205k)</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#ea580c;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱150,000 – ₱204,000 / sqm</span></div><div class="rate-tier-desc">One Shang (₱179k), St. Francis (₱168k), Twin Oaks (₱158k), Fame (₱155k), Zitan (₱153k)</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#eab308;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱105,000 – ₱149,000 / sqm</span></div><div class="rate-tier-desc">Avida Centera (₱135k), Urban Deca (₱128k), Soho Central (₱110k), Grand Central (₱108k)</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#84cc16;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱56,000 – ₱104,000 / sqm</span></div><div class="rate-tier-desc">California Garden (₱75k), Diamond (₱75k), Ark (₱75k), Baseline (₱56k)</div></div></div>
        ...
        tiers.innerHTML = `
          ...
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#059669;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱110,000 – ₱179,000 / sqm</span></div><div class="rate-tier-desc">One Sierra (₱144k), One Shang (₱125k), St. Francis (₱118k), Twin Oaks (₱111k), Fame (₱109k)</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#10b981;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱75,000 – ₱109,000 / sqm</span></div><div class="rate-tier-desc">Zitan (₱107k), Tektite (₱98.7k), Avida Centera (₱95k), Urban Deca (₱90k), Grand Central (₱76k)</div></div></div>
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#34d399;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>₱40,000 – ₱74,000 / sqm</span></div><div class="rate-tier-desc">Lancaster (₱74k), California Garden (₱53k), Summit One (₱58k), Baseline (₱40k)</div></div></div>
        ...
        tiers.innerHTML = `
          ...
          <div class="rate-tier-row"><span class="rate-tier-swatch" style="background:#2563eb; border:2px solid #1e3a8a;"></span><div class="rate-tier-info"><div class="rate-tier-val"><span>Government Land (GL)</span></div><div class="rate-tier-desc">DepEd Complex, Brgy Highway Hills &amp; San Antonio Halls, Police &amp; Fire Stations</div></div></div>
    ...
    function renderActiveLayerSummary(mode) {{
      ...
      if (mode === 'CR') box.innerHTML = '<strong>Commercial Regular (CR)</strong> covers raw land/lot valuations along commercial streets. Rates range from ₱45k in interior streets up to ₱325k / sqm on major thoroughfares (ADB Ave, San Miguel, Julia Vargas, EDSA).';
      else if (mode === 'CC') box.innerHTML = '<strong>Commercial Condominium (CC)</strong> values individual office units in commercial towers. Rates range from ₱70k baseline up to ₱315k / sqm (Offices at The Galleon). Pure residential parcels are greyed out and unclickable.';
      else if (mode === 'RR') box.innerHTML = '<strong>Residential Regular (RR)</strong> applies to residential lot parcels. Ranges from ₱34k in interior Highway Hills up to ₱200k / sqm along the EDSA Mandaluyong corridor.';
      else if (mode === 'RC') box.innerHTML = '<strong>Residential Condominium (RC)</strong> applies to living dwelling units in towers. Rates range from ₱56k baseline up to ₱275k / sqm (Residences at The Galleon). Pure commercial buildings (e.g. Megamall, Tektite) are greyed out and unclickable.';
      else if (mode === 'PS') box.innerHTML = '<strong>Parking Slot (PS)</strong> represents parking spaces assessed at the BIR statutory 70% rule of unit value. Rates range from ₱40k to ₱220.5k / sqm.';
    ...
    document.getElementById('chk-hh').addEventListener('change', e => {{ if (e.target.checked) map.addLayer(hhLayer); else map.removeLayer(hhLayer); }});
